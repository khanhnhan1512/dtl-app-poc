...
`kill-command.json` is generated by `lab/setup.sh` (a fresh signed no-op every run) and is not
committed - copy one of the signed samples below over it to change what the app's next poll sees.
